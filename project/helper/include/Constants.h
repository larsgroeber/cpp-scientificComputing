...
// size of masspoints (radius) (should be ~ n * ( 2 * rm )² < pi * rp² = A => rm < sqrt( pi * rp² * / n ) * 1/2 ) [ rm = MASSPOINTS_RADIUS , rp = ASTEROID_RADIUS_START ]
const long double MASSPOINTS_RADIUS = .1;

// starting positon of the asteroid
const LH::Vector ASTEROID_POS_START ( 0 , 8 );
